...
int32_t FunctionServlet::handle(sylar::http::HttpRequest::ptr request,
    ...
    return m_cb(request, response, session); // 直接调用存储的 callback 处理 HTTP 请求
}
...
void ServletDispatch::addServlet(const std::string &uri, Servlet::ptr slt) {
    ...
    // uri 作为键，将 Servlet 存入 m_datas，用于精确匹配
...
void ServletDispatch::addServletCreator(const std::string &uri, IServletCreator::ptr creator) {
    RWMutexType::WriteLock lock(m_mutex);
    m_datas[uri] = creator; // 直接传入 callback，包装成 FunctionServlet 并存入 m_datas
}
...
// 处理 404 错误，请求的 URL 没有匹配的 Servlet 时返回
