...
HttpResponse::ptr HttpConnection::recvResponse() {
    /// 1.创建 HTTP 响应解析器 并 初始化
    HttpResponseParser::ptr parser(new HttpResponseParser);
    uint64_t buff_size = HttpRequestParser::GetHttpRequestBufferSize();
    //uint64_t buff_size = 100;   // 分配缓冲区
    std::shared_ptr<char> buffer(
            new char[buff_size + 1], [](char* ptr){
                delete[] ptr;
            });
    char* data = buffer.get();
    int offset = 0;   // 追踪 buffer 的数据偏移量
    /// 2.循环读取 HTTP 响应头
    do {
        ...
        len += offset;
        data[len] = '\0';
        size_t nparse = parser->execute(data, len, false); // 解析HTTP头部
        if(parser->hasError()) {  // 解析错误则关闭连接
            close();
            return nullptr;
        }
        offset = len - nparse;
        ...
        if(parser->isFinished()) { // 解析完成，跳出循环
            break;
        }
    } while(true);
    /// 3.解析 HTTP 响应体
    auto& client_parser = parser->getParser(); //解析结果
    std::string body; // 存储 HTTP 响应体
    if(client_parser.chunked) {  // 3.1 处理 Chunked 传输编码 逐块读取
        int len = offset;
        do {
            bool begin = true;
            do {
...
int HttpConnection::sendRequest(HttpRequest::ptr rsp) {
    std::stringstream ss;
    ss << *rsp;
    std::string data = ss.str();  // 将 HttpRequest 序列化为字符串
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method
        ...
        , const std::map<std::string, std::string>& headers
        , const std::string& body) {
    HttpRequest::ptr req = std::make_shared<HttpRequest>(); //创建一个HttpRequest对象req
    // 设置路径、查询参数、片段、请求方法、请求头、请求体
    ...
    req->setBody(body);
    return DoRequest(req, uri, timeout_ms);  // 具体请求
}
