...
/// TcpServerConf 结构体，用于存储 TCP 服务器的配置
struct TcpServerConf {
    typedef std::shared_ptr<TcpServerConf> ptr;

    std::vector<std::string> address; // 服务器监听的地址
    int keepalive = 0;                // 是否启用 keepalive
    int timeout = 1000 * 2 * 60;      // 连接超时时间（默认 2 分钟）
    int ssl = 0;                      // 是否启用 SSL
    std::string id;                   // 服务器 ID
    std::string type = "http";        // 服务器类型（http, ws, rock）
    std::string name;                 // 服务器名称
    std::string cert_file;            // SSL 证书文件路径
    std::string key_file;             // SSL 私钥文件路径
    std::string accept_worker;        // 负责 `accept()` 的协程线程池
    std::string io_worker;            // 负责 IO 操作的协程线程池
    std::string process_worker;       // 负责业务处理的协程线程池
    std::map<std::string, std::string> args;  // 额外参数
...
/// TcpServerConf 转 字符串  构造 YAML Node
template<>
